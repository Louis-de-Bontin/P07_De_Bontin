...
def get_best_folio(actions, capital):
    ...
    '''
    folio = []
    # Création de la matrice avec une action 0 pour avoir
    # une ligne de réference
    ...
    for action in actions:
        ...
        while cap <= capital:
            ...
            if action != "action0":
                # Je compare l'ancien meilleur porte feuille et
                # le nouveau et garde le meilleur
                # En gardant bien le ROI (pour comparer) et la
                # liste d'action (ce qui nous interesse)
                folio = [action]
...
def display_results(best_folio):
    ...
    print(
        ...
        str((best_folio[0]/total_cost*100)/10000) +
        "%."
    )
